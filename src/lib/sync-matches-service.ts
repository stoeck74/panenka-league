...
// ============================================
// SYNC SERVICE — logique partagée
// ============================================
// Extrait ici pour être appelée depuis DEUX endroits :
//   1. app/api/sync-matches/route.ts   (cron externe, protégé par CRON_SECRET)
//   2. app/api/admin/sync-matches/route.ts (bouton admin, protégé par la session)
//
// Avant, cette logique n'existait que dans la route cron, et le bouton admin
// devait donc rappeler cette même route via un token — token qui a fini par
// fuiter côté client (NEXT_PUBLIC_CRON_SECRET). En la centralisant ici, la
// route admin peut déclencher exactement la même sync sans jamais exposer
// le secret cron.
...
export async function runMatchesSync(forceRecalc: boolean): Promise<SyncResult> {
